...
export function ReportsView({ user }: ReportsViewProps) {
  ...
  const hasMonthlyData =
    data?.monthlyTrends &&
    data.monthlyTrends.some((t: any) => t.income > 0 || t.expense > 0);
  const hasCategoryData = data?.categoryBreakdown && data.categoryBreakdown.length > 0;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8 animate-tab-switch pb-24 md:pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 no-print">
          <button
            ...
            className="btn-secondary flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-xl"
          >
            <Printer className="h-3.5 w-3.5 text-cyan-400" />
            <span>Print / PDF Report</span>
          </button>

          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/[0.04] border border-white/[0.08] text-emerald-400 text-xs font-semibold">
      ...
      {loading ? (
      ...
      ) : !data || (!hasMonthlyData && !hasCategoryData) ? (
        <div className="py-16 text-center glass-card rounded-3xl p-8 space-y-3">
          <div className="h-12 w-12 rounded-2xl bg-white/[0.04] border border-white/[0.08] text-neutral-500 flex items-center justify-center mx-auto">
            <BarChart3 className="h-6 w-6" />
          </div>
          <h3 className="text-base font-bold text-white">No Financial History Available</h3>
          <p className="text-xs text-neutral-400 max-w-sm mx-auto leading-relaxed">
            Log transactions in your Personal Finance ledger to unlock annual cash flow analytics and category distribution charts.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="glass-card rounded-3xl p-5 sm:p-7">
            ...
            {!hasMonthlyData ? (
              <div className="h-60 flex flex-col items-center justify-center text-center p-6 border border-dashed border-white/[0.06] rounded-2xl bg-[#090a0d]">
                <p className="text-xs text-neutral-400">No monthly cash flow data recorded</p>
              </div>
            ) : (
          ...
          <div className="glass-card rounded-3xl p-5 sm:p-7">
            ...
            {!hasCategoryData ? (
              <div className="h-60 flex flex-col items-center justify-center text-center p-6 border border-dashed border-white/[0.06] rounded-2xl bg-[#090a0d]">
                <p className="text-xs text-neutral-400">No category expense data recorded</p>
              </div>
            ) : (
